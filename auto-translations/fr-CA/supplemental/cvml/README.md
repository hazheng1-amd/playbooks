...
La [bibliothèque Ryzen AI CVML](https://ryzenai.docs.amd.com/en/latest/ryzen_ai_libraries.html#ryzen-ai-cvml-library) est une boîte à outils AMD en C++ pour la vision par ordinateur et l'apprentissage automatique, offrant de puissantes capacités de perception sur l'appareil, notamment l'estimation de profondeur, la détection de visages et le suivi de maillage facial. Construite au-dessus des pilotes Ryzen AI, la bibliothèque sélectionne automatiquement le meilleur matériel disponible (GPU ou NPU) pour l'inférence, vous permettant d'ajouter des fonctionnalités d'IA à des applications C++ sans avoir à vous soucier de l'entraînement des modèles ou de l'intégration de cadriciels. Tout le traitement s'effectue localement sur votre système, ce qui la rend idéale pour les applications sensibles à la confidentialité et à faible latence.

Ce guide vous apprend à configurer la bibliothèque Ryzen AI CVML, à compiler les exemples d'applications inclus et à exécuter la détection de visages sur une image d'exemple.

## Ce que vous allez apprendre

- Comment installer les prérequis et configurer la bibliothèque Ryzen AI CVML sur votre système
- Comment fonctionne l'API C++ de CVML : contextes, objets de fonctionnalité et tampons d'image
- Comment compiler et exécuter les exemples d'applications inclus à l'aide de CMake et d'OpenCV
- Comment exécuter la détection de visages sur une image avec des cadres de délimitation et des points de repère
- Comment intégrer les fonctionnalités de CVML dans vos propres applications C++
...
<!-- @os:windows -->
- [OpenCV 4.11](https://github.com/opencv/opencv/releases/tag/4.11.0) — téléchargez `opencv-4.11.0-windows.exe`, exécutez-le et extrayez son contenu dans un dossier local (p. ex. `C:\opencv`)
- [CMake](https://cmake.org/download/) — téléchargez l'installeur MSI Windows x86-64 et, durant l'installation, sélectionnez **« Add CMake to the system PATH for all users »**
- [Pilote NPU Ryzen AI](https://ryzenai.docs.amd.com/en/latest/inst.html) — installez la dernière version disponible
- [Visual Studio 2022 Community](https://aka.ms/vs/17/release/vs_community.exe) avec la charge de travail « Développement Desktop en C++ » (comprend le compilateur MSVC, le SDK Windows et les outils de compilation C++)
<!-- @os:end -->

<!-- @os:linux -->
- OpenCV 4.11 — doit être compilé à partir des sources (les paquets apt sur Ubuntu 22.04 et 24.04 ne fournissent pas la version 4.11). Consultez [Compiler OpenCV à partir des sources](#building-opencv-from-source) ci-dessous.
- CMake — installez-le via apt :
  ```bash
  sudo apt install cmake
  ```
- Ubuntu 22.04 ou 24.04 (noyau >= 6.11.0-21-generic)
- [Pilote NPU Ryzen AI](https://ryzenai.docs.amd.com/en/latest/linux.html#install-npu-drivers) (installeur Linux — requis pour l'inférence NPU)
...
Créez un compte AMD à l'adresse [account.amd.com](https://account.amd.com) si vous n'en avez pas, puis connectez-vous pour télécharger la bibliothèque Ryzen AI CVML à partir du lien du portail ci-dessous :

```
https://account.amd.com/en/forms/downloads/xef.html?filename=72293_Ryzen_AI_Library_26.05.20.zip
```

Une fois le téléchargement terminé, extrayez le paquet dans un répertoire local (p. ex. `C:\RyzenAI-Library` sous Windows ou `~/RyzenAI-Library` sous Linux) et définissez la variable d'environnement `AMD_CVML_SDK_ROOT` avec l'emplacement d'extraction :
...
Chaque application CVML suit le même modèle en quatre étapes :

1. **Créer un contexte** — L'objet `amd::cvml::Context` gère les ressources partagées comme la journalisation et la sélection du moteur d'inférence.
2. **Créer un objet de fonctionnalité** — Instanciez la fonctionnalité spécifique (p. ex., `amd::cvml::DepthEstimation`) à partir du contexte.
...
La bibliothèque sélectionne automatiquement le meilleur matériel (GPU ou NPU) pour chaque opération. Vous pouvez aussi définir le backend explicitement :
...
> **Remarque :** Les fonctionnalités utilisant le backend ONNX pour les opérations NPU peuvent présenter une latence de démarrage plus longue lors de la première exécution. Les exécutions suivantes seront plus rapides.
...
La bibliothèque CVML comprend des exemples d'applications prêts à être compilés pour chaque fonctionnalité. Compilons-les tous en une seule fois.
...
2. Compilez les exemples avec CMake :
   ...
   Une fois la compilation réussie, les exécutables se trouvent dans :
...
L'exemple de détection de visages détecte les visages dans une image, une vidéo ou un flux de caméra en direct. Il dessine des cadres de délimitation, des scores de confiance et cinq points de repère faciaux (deux yeux, un nez et deux coins de bouche) sur chaque visage détecté.
...
Une fenêtre s'affiche montrant l'image avec des cadres de délimitation autour des visages détectés, des scores de confiance et des points de repère faciaux (yeux, nez, coins de bouche).
...
|-------|-------|----------|----------|
| `fast` (par défaut) | FPS plus élevé | Bonne | Applications de caméra en temps réel |
| `precise` | FPS plus faible | Meilleure | Analyse de photos, besoins de haute précision |
...
Pour chaque exemple ci-dessous, accédez d'abord à son dossier exécutable, en suivant le même schéma que la section [Running Face Detection](#running-face-detection) ci-dessus (par exemple, `cd build\cvml-sample-depth-estimation\Release` sous Windows ou `cd build/cvml-sample-depth-estimation` sous Linux). Sous Windows, ajoutez `.exe` à chaque commande (par exemple, `cvml-sample-depth-estimation.exe`).

- **Essayer l'estimation de la profondeur** : exécutez `cvml-sample-depth-estimation -i sample_face.jpg` pour générer une carte de profondeur colorisée — les objets plus proches apparaissent en couleurs chaudes, les plus éloignés en couleurs froides
- **Explorer le maillage facial**: exécutez `cvml-sample-face-mesh -i sample_face.jpg` pour voir le suivi dense de la géométrie faciale avec des points de maillage détaillés
...
- **Comparer les variantes de modèle** : essayez `-m precise` par rapport à l'option par défaut `-m fast` sur la détection de visage pour observer directement le compromis entre précision et vitesse
...
- **Combiner des fonctionnalités** : enchaînez la détection de visage avec l'estimation de la profondeur dans la même application pour une compréhension plus riche de la scène
- **Parcourir le code source** : consultez la [bibliothèque Ryzen AI CVML sur GitHub](https://github.com/amd/RyzenAI-SW/tree/main/Ryzen-AI-CVML-Library) pour la documentation des en-têtes, des exemples supplémentaires et les détails de l'API